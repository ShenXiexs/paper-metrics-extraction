# 0414 输出结果的 Prompt 说明

本文档对应我做实验时实际用到的指令和对应的prompt
...
- `system prompt` 使用脚本中的 `SYSTEM_PROMPTS["cn"]`
- `user prompt` 使用脚本中的 `build_text_full_mode_prompt(..., prompt_language="cn")`
- 实际请求时会把这两段一起发给模型

## 1. System Prompt

给模型设边界和行为规范，它定义的是长期不变的抽取规则
...
5. 用上面的 `system prompt + user prompt` 一起发给模型

## 5. 附录：英文版对应指令和Prompt（我测了英文的前30条和中文前30条的结果，结果是一样的）
...
python3 scripts/extract_metrics_batch.py \
  ...
  --input-mode text_full \
  --prompt-language en \
  --run-name codexa_textfull_en_full \
  --char-budget 150000
```

### 5.1. English System Prompt
...
### 5.2. English User Prompt Template
